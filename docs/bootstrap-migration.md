...
- **Compile the custom theme**: Author layout + component styling in `_theme.scss`, import tokens via `ferraen.scss`, and compile to `static/styles/app.css` (run `npm run build` once npm access is restored, or any Sass compiler) so Django serves a single optimized stylesheet.
